...
export function tokenize(source: string, filePath?: string): Token[] {
  ...
  try {
    ...
    throw SurfstarError.lexerError(
      "Error during tokenization",
      { filePath, originalError: error instanceof Error ? error : undefined }
    );
  }
}
...
function validateTokens(tokens: Token[], filePath?: string): void {
    ...
    if (token.type === TokenType.CLOSE_BRACE) {
      ...
      if (openBraceCount < 0) {
        throw SurfstarError.lexerError(
          ...
          { filePath, lineNumber: token.line, columnNumber: token.column }
        );
      }
    }
  ...
  if (openBraceCount > 0) {
    ...
    throw SurfstarError.lexerError(
      "Unclosed braces in template",
      { filePath, lineNumber: lastToken.line, columnNumber: lastToken.column }
    );
  }
  ...
  if (eachBlockCount > 0) {
    const lastToken = tokens[tokens.length - 1];
    throw SurfstarError.lexerError(
      "Unclosed {{#each}} block in template",
      { filePath, lineNumber: lastToken.line, columnNumber: lastToken.column }
    );
  }
}
...
function isEachStart(source: string, index: number): boolean {
  return (
    ...
    source.slice(
      index + lexerIndexes.eachStart.wordBegin,
      index + lexerIndexes.eachStart.end,
    ) === "each"
  );
}
...
function isEachEnd(source: string, index: number): boolean {
  return (
    source[index + 2] === "/" && source.slice(index + 3, index + 7) === "each"
  );
}
...
function validateEachArrayName(arrayName: string, line: number, column: number, filePath?: string): void {
  if (!arrayName) {
    throw SurfstarError.lexerError(
      "Each block is missing array name",
      { filePath, lineNumber: line, columnNumber: column }
    );
  }
}
